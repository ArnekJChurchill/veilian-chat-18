...
function banUser() {
  ...
  fetch("/ban", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username }) })
    .then(res => res.json())
    .then(data => { if (data.success) alert(`User @${username} has been banned.`); document.getElementById("banUserInput").value = ""; });
}
...
function unbanUser() {
  ...
  fetch("/unban", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username }) })
    .then(res => res.json())
    .then(data => { if (data.success) alert(`User @${username} has been unbanned.`); document.getElementById("unbanUserInput").value = ""; });
}
